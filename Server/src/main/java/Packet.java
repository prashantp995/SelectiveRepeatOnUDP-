...
/**
 * Packet represents a simulated network packet. As we don't have unsigned types in Java, we can
 * achieve this by using a larger type.
 */
public class Packet {
  ...
  public Packet(int type, long sequenceNumber, InetAddress peerAddress, int peerPort,
      byte[] payload) {
    this.type = type;
  ...
  /**
   * Creates a builder from the current packet. It's used to create another packet by re-using some
   * parts of the current packet.
   */
  ...
  /**
   * Writes a raw presentation of the packet to byte buffer. The order of the buffer should be set
   * as BigEndian.
   */
  ...
  /**
   * Create a byte buffer in BigEndian for the packet. The returned buffer is flipped and ready for
   * get operations.
   */
  ...
  public String toString() {
    return String
        .format("#%d peer=%s:%d, size=%d ,timestart =%d", sequenceNumber, peerAddress, peerPort,
            payload.length, timestart);
  }
